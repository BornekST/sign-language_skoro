...
def _normalize_single(landmarks: list[list[float]]) -> Optional[np.ndarray]:
    """
...
def normalize_landmarks(hand_data: dict) -> Optional[np.ndarray]:
    """
    ...
    if hand_data["right"] is not None:
        r = _normalize_single(hand_data["right"])
        if r is not None:
            right_features = r

    if hand_data["left"] is not None:
        lf = _normalize_single(hand_data["left"])
        if lf is not None:
            left_features = lf
